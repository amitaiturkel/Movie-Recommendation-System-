...
struct movieComparator {
    bool operator()(const std::shared_ptr<Movie>& m1, const std::shared_ptr<Movie>& m2) const {
      // Custom comparison logic for movies
      return (*m1) < (*m2);
    }
};
...
{
 private:
  std::map<const sp_movie, std::vector<double>, movieComparator> sortedmap;
 public:
